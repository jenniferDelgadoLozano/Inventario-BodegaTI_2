...
class  Media {
  ...
  public $actaPath = SITE_ROOT.DS.'..'.DS.'uploads/actas';
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
   ...
   /*--------------------------------------------------------------*/
 /* Función para procesar archivo acta
 /*--------------------------------------------------------------*/
 public function process_acta(){
  if(!empty($this->errors)){
      return false;
    }
  if(empty($this->fileName) || empty($this->fileTempPath)){
  ...
  if(!is_writable($this->actaPath)){
      $this->errors[] = $this->actaPath." Debe tener permisos de escritura!!!.";
      return false;
    }

  if(file_exists($this->actaPath."/".$this->fileName)){
    $this->errors[] = "El archivo {$this->fileName} Realmente existe.";
    return false;
  }

  if(move_uploaded_file($this->fileTempPath,$this->actaPath.'/'.$this->fileName))
  {
    if($this->insert_acta()){
    unset($this->fileTempPath);
    return true;
...
/*--------------------------------------------------------------*/
/* Function for insert acta image
/*--------------------------------------------------------------*/
private function insert_acta(){

  global $db;
  $sql  = "INSERT INTO actas ( titulo,nombrearchivo )";
  $sql .=" VALUES ";
  $sql .="(
           '{$db->escape($this->fileName)}',
           '{$db->escape($this->fileType)}'
